...
function Calendar(element, options, eventSources) {
	...
	function updateEvents(forceRender) {
		if (!options.lazyFetching || isFetchNeeded(currentView.visStart, currentView.visEnd)) {
			//console.info('calling refetchEvents()');
			refetchEvents();
		}
		else if (forceRender) {
			//console.info('calling rerenderEvents()');
			rerenderEvents();
		}
	}
	...
	function refetchEvents() {
		//console.info('refetchEvents(' + currentView.visStart + ','+ currentView.visEnd +')');		
	...
	//console.info("reportEvents1:" + _events.length);
		events = _events;
		rerenderEvents();
	}
		...
		if (buttonStr) {
			...
			$.each(buttonStr.split(' '), function(i) {
				if (i > 0) {
					...
					tr.append("<td width='1px'><span class='fc-header-space'/></td>");
				}
				var prevButton;
				$.each(this.split(','), function(j, buttonName) {
					if (buttonName == 'title') {
						//tr.append("<td><h2 class='fc-header-title'>&nbsp;</h2></td>");
						tr.append("<td width='210px'><h2 class='fc-header-title'>&nbsp;</h2></td>");
						if (prevButton) {
						...
						if (buttonClick) {
							...
							if (icon) {
								...
								button = $("<div class='fc-button-" + buttonName + " ui-state-default'>" +
										"<a><img src='/images/Transparent.gif'/></a></div>"); //prev|next
							}
							if (button) {
								button
									.click(function() {
...
function EventManager(options, sources) {
	...
	function fetchEvents(start, end) {
		//console.info('in fetchEvents:sources.length=	 ' + sources.length)
		rangeStart = start;
		rangeEnd = end;
		cache = [];
		...
		for (var i=0; i<len; i++) {
			 //console.info('sources['+i+']='+sources[i])
			 //console.info('fetchID='+fetchID)
	...
	function fetchEventSource(source, fetchID) {
		_fetchEventSource(source, function(events) {
			if (fetchID == currentFetchID) {
				//console.info('fetchEventSource:events.length='+ events.length);
	...
	function _fetchEventSource(source, callback) {
		if (typeof source == 'string') {
			//console.info('_fetchEventSource:1')
			var params = {};
			...
			//console.info('_fetchEventSource:2')
			pushLoading();
			...
			//console.info('_fetchEventSource:3')
...
//console.info("renderEvent");
//console.info(event);
//console.info(stick);
		normalizeEvent(event);
		//console.info ('event.source ' + event.source +":"+stick);			
		if (!event.source) {
...
function BasicView(element, calendar, viewName) {
		...
		if (!tbody) { // first time, build all cells from scratch
			...
			for (i=0; i<colCnt; i++) {
				s += "<th class='fc-" +
					...
					tm + '-state-default' +
					(i==dit ? ' fc-leftmost' : '') + 
					" cal-day-header'>" + formatDate(d, colFormat) + "</th>";
				addDays(d, 1);
				if (nwe) {
					skipWeekend(d);
				}
			...
			s = "<tbody class='cal-content-number'>"; //e h o
			...
			for (i=0; i<rowCnt; i++) {
				...
				for (j=0; j<colCnt; j++) {
					s += "<td class='fc-" +
						...
						' fc-not-today') + "' id='fc-day-" + formatDate(d, 'yyyyMMdd') +"'>" +
				...
				for (i=prevRowCnt; i<rowCnt; i++) {
					...
					for (j=0; j<colCnt; j++) {
						s += "<td class='fc-" +
							...
							(j==dit ? ' fc-leftmost' : '') + "' id='fc-day-" + formatDate( d,'yyyyMMdd') +"'>" +
			...
			tbody.find('td').each(function() {
				...
				td.attr('id', 'fc-day-' + formatDate(d,'yyyyMMdd')); /* e h o */
				addDays(d, 1);
				if (nwe) {
					skipWeekend(d);
				}
	...
	function setHeight(height) {
	    ...
	    //console.info('rowHeight1:rowHeight2='+ rowHeight1 + ":" + rowHeight2);
		//rowHeight1 = rowHeight2 = 101;
	...
	function dayClick(ev) {
		...
		if (!opt('selectable')) { // SelectionManager will worry about dayClick
			...
			return trigger('dayClick', this, date, true, ev);
			//return trigger('dayClick', this, ev, false);
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			...
			if (eventElement = seg.element) {
				...
				//console.info('eventElement[0].style.height=' + eventElement[0].style.height)
				event = seg.event;
...
function View(element, calendar, viewName) {
	...
	function reportEvents(events) { // events are already normalized at this point
		//console.log("reportEvents2:" + events.length);
		eventsByID = {};
	...
	function eventElementHandlers(event, eventElement) {
		eventElement.draggable = undefined;
		eventElement
			.click(function(ev) {
				if (!eventElement.hasClass('ui-draggable-dragging') &&
					!eventElement.hasClass('ui-resizable-resizing')) {
					$(this).removeClass('ui-draggable ui-draggable-dragging');
						//console.info('eventClick for ' + $(this).attr('id'))
						return trigger('eventClick', this, event, ev);
					}
			})
			.hover(
				function(ev) {
					$(this).removeClass('ui-draggable ui-draggable-dragging');
					//console.info('eventMouseover for ' + $(this).attr('class'))
					//console.info('eventMouseover for ' + $(this).attr('id'))
					trigger('eventMouseover', this, event, ev);
				},
				function(ev) {
					$(this).removeClass('ui-draggable ui-draggable-dragging');
					//console.info('eventMouseout for ' + $(this).attr('id'))
...
function DayEventRenderer() {
	...
	function renderDaySegs(segs, modifiedEventId) {
		...
		//2012 console.info('renderDaySegs:'+modifiedEventId+":"+segs);
		//2012 console.info('segmentContainer[0].innerHTML:'+segmentContainer[0].innerHTML)
		//console.info('segmentContainer[0].innerHTML = '+segmentContainer[0].innerHTML );
		//console.info(rowCnt + ':' +colCnt + ':' +segCnt)
...
/*
				...
				seg.top = top;
				top += seg.outerHeight;
\\				console.info('height for row ' +rowI +'=' + top);
*/
			...
			//console.info('rowDivs['+rowI+'].height=' + rowDivs[rowI].height)
		}
	...
	function renderTempDaySegs(segs, adjustRow, adjustTop) {
		...
		var segCnt = segs.length;
		var element;
		//2012 console.info('renderTempDaySegs:'+adjustRow+":"+adjustTop);
		...
		for (i=0; i<segCnt; i++) {
			...
			/*html +=
				"<div class='" + className + event.className.join(' ') + "' style='position:absolute;left:"+left+"px'>" +
					"<a style='' " + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
						...
						:'') +
						//"<span class='fc-event-title'>" + htmlEscape(event.title) + "</span>" +
						//console.debug('event.title ' + event.title);
						//"<div style='overflow:visible;width:"+event.width +"px; height:"+event.height +";' class='"+event.title+"'>&nbsp;</div>" +
						"<img src='"+event.image+"'/>"+
					"</a>" +
			...
			var top = 0;
			var debug = null;
			  if (event.className == 'media-planner-event' ) 
			  {
				  extraLeft= left + 55;
				  top = 44;
			  }
			  else if (event.className == 'event-planner-event' )
			  {
				  extraLeft=  left + 55;
				  top = 11;
			  }
			  else if (event.className == 'miscellaneous-planner-event' )
			  {
				  debug = 1;
				  extraLeft=  left + 31;
				  top = -15;
			  }else if(event.className == 'news-planner-event') {
					  top = 45;
					  extraLeft = left + 8;
			  } else { //dm
					  top = 11;
			  ...
			  var estyle = ''; 
			  if( $('#calendar').fullCalendar( 'getDate' ).getMonth() != event.start.getMonth() )
				  estyle = ';filter:alpha(opacity=30); opacity:0.30'
			var frag =
				//"<div class='" + className + event.className.join(' ') + "' style='position:absolute;left:"+left+"px'>" +
					"<a style='position:absolute;top:"+top +"px;left:"+extraLeft+"px" + estyle+"' " 
							+ (event.url ? " href='" + htmlEscape(event.url) + "'" : '#') + " class='ui-draggable-disabled'>" +
						(!event.allDay && seg.isStart ?
							"<span class='fc-event-time'>" +
								htmlEscape(formatDates(event.start, event.end, opt('timeFormat'))) +
							"</span>"
						...
						//console.debug('event.title ' + event.title);
						//"<div style='overflow:visible;width:"+event.width +"px; height:"+event.height +";' class='"+event.title+"'>&nbsp;</div>" +
						"<img src='"+event.image+"' class='"+event.className+"'/>"+
					"</a>"// +
				//	(seg.isEnd && (event.editable || event.editable === undefined && opt('editable')) && !opt('disableResizing') ?
					//	"<div class='ui-resizable-handle ui-resizable-" + (rtl ? 'w' : 'e') + "'></div>"
					//	: '') +
	...
	/*	
		...
		for (i=0; i<segCnt; i++) {
			...
			if (element) {
				...
				//console.info('element[0].style.width=' + element[0].style.width );
			}
		}
	*/
	}
	...
	function daySegSetTops(segs, rowTops) { // also triggers eventAfterRender
		...
		for (i=0; i<segCnt; i++) {
			...
			if (element) {
				...
				// e h o element[0].style.top = rowTops[seg.row] + (seg.top||0) + 'px';
				//console.info('rowTops[seg.row]=' +rowTops[seg.row]);
						////console.info('element[0].style.height='+element[0].style.height);
				var top = parseInt(element[0].style.top);
				//2012 console.info('top:'+top+':' +rowTops[seg.row])
				element[0].style.top = rowTops[seg.row] + (top||0) + 'px';
				event = seg.event;
...
function setOuterWidth(element, width, includeMargins) {
	element.each(function(i, _element) {
		...
		//console.info(_element.id +':'+_element.class)
		_element.style.width =  '100px';
		//console.info(width +':' + includeMargins);
		//console.info('_element.style.width=' + _element.style.width);
	});
}
...
function setOuterHeight(element, height, includeMargins) {
	element.each(function(i, _element) {
		...
		//console.info('_element.style.height=' + _element.style.height);
	});
}
